...
<form id="refreshPage" action="adminDocumentExport.php" method="POST">
...
<div class="row">
	...
	<?php
		if (empty($_POST['command'])) {
			$leftCnt = $conn->query("SELECT count(1) from documents where revision_comments='Zaakceptowane' and completion_date='0000-00-00 00:00:00' and pesel in (select pesel from kwalifikacje where komunikat like 'przyjęty%')")->fetch()[0];
		}	
	?>
	<div class="panel panel-default" <?php if (!empty($_POST['command']) || $leftCnt==0) { echo ' style="display: none;" '; }  ?> >
			  <div class="panel-heading">EKSPORT ZATWIERDZONYCH ŚWIADECTW DOJRZAŁOŚCI<p>Liczba dokumentów do eksportu: <?php echo $leftCnt;?></p></div>
	...
	<div <?php if (!empty($_POST['command']) || $leftCnt!=0) { echo ' style="display: none;" '; } ?> >
		<div class="alert alert-danger" role="alert">
			<strong>Och!</strong> Nie znaleziono nowych, zatwierdzonych świadectw dojrzałości do eksportu.
		</div>	
	...
	<?php
		if ($_POST['command']=='COMMAND_CONTINUE') {
			...
			if (!mkdir('D:/xampp2/htdocs_data/documents/'.$folderName, 0700, true)) {
				 echo '<div class="alert alert-danger" role="alert">
			...
			$myFileName = 'D:/xampp2/htdocs_data/documents/'.$folderName.'/'.'copyFiles.bat';
			$myfile =fopen($myFileName,"w") or die("Unable to open file!");			
			for ($i = 0; $i <= count($arr_A)-1; $i++) {
				$pathFrom = 'D:/xampp2/htdocs_data/documents/' .$arr_A[$i][pesel]. '.pdf';
				$pathTo = 'D:/xampp2/htdocs_data/documents/'.$folderName.'/'.$arr_A[$i][pesel]. '.pdf';
			...
			try{ $stmt->execute(['completion_date' => $data_godz]); }
				catch(PDOException $e){ die('An error occured: '.$e); };
	...
	<div class="panel panel-default" <?php if ($_POST['command']=='COMMAND_CONTINUE') {} else { echo ' style="display: none;" '; }  ?> >
			  ...
			  <div class="panel-body">
				...
				<p>Dokumenty są właśnie kopiowane do tego folderu.</p>
				<p>Odczekaj kilka minut by mieć pewność, że wszystkie dokumenty zostały skopiowane.</p>
				<button type="button" class="btn btn-default glyphicon glyphicon-home" onclick="self.location.href=('./index.php');"> Powrót</button>						
			  </div>
	</div>	
	...
	<div class="panel panel-default" <?php if ($_POST['command']=='COMMAND_REPROCESS') {} else { echo ' style="display: none;" '; }  ?> >
			  <div class="panel-heading">Ponowne eksportowanie wszystkich dokumentów</div>
			  <div class="panel-body">
				<p>Informacja na temat wcześniejszych eksportów  została usunięta, możesz teraz ponownie wyeksportować wszystkie dokumenty!</p>
				<button type="button" class="btn btn-success glyphicon glyphicon-ok" onclick="document.getElementById('command').value='COMMAND_CONTINUE'; document.getElementById('refreshPage').submit();"> Kontynuuj</button>		
			  </div>
	</div>	
